...
export const PurgeConfirmModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white border border-slate-100 w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-fade-in">

        {/* Header */}
        <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100 bg-rose-50/30">
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              >
                <div className={`p-2.5 rounded-xl self-start ${purgeType === 'transactions' ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-500'
                  }`}>
                  <ListFilter size={18} />
                </div>
                ...
                <div className="self-center">
                  <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${purgeType === 'transactions' ? 'border-rose-500 bg-rose-500' : 'border-slate-300'
                    }`}>
                    {purgeType === 'transactions' && <div className="w-2 h-2 rounded-full bg-white" />}
                  </div>
                </div>
              </div>
              ...
              >
                <div className={`p-2.5 rounded-xl self-start ${purgeType === 'all' ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-500'
                  }`}>
                  <Database size={18} />
                </div>
                <div className="flex-1">
                ...
                <div className="self-center">
                  <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${purgeType === 'all' ? 'border-red-600 bg-red-600' : 'border-slate-300'
                    }`}>
                    {purgeType === 'all' && <div className="w-2 h-2 rounded-full bg-white" />}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-2 bg-slate-50 border border-slate-100 rounded-2xl p-4">
            ...
            <div className="my-1.5 py-1 px-3 bg-slate-200/60 rounded-lg select-all text-slate-800 font-extrabold text-xs text-center border border-slate-300/40 select-none">
              {targetWord}
            </div>

            <input
              type="text"
              className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-slate-800 text-sm font-semibold outline-none focus:border-rose-400 transition-all uppercase placeholder-slate-400"
