        ...
        <div className="absolute left-0 mt-1.5 w-48 bg-white border border-slate-100 rounded-2xl shadow-xl py-1.5 z-50 max-h-60 overflow-y-auto animate-fade-in">
          ...
          {txCategories.map(cat => (
            <button
              ...
              className={`w-full px-3.5 py-2 text-left hover:bg-slate-50 flex items-center gap-2 text-[11px] font-bold transition-colors cursor-pointer ${tx.category_id === cat.id ? 'bg-indigo-50/40 text-indigo-600' : 'text-slate-650 hover:text-slate-800'
                }`}
            >
              <span
...
export const TransactionTable = () => {
  ...
  return (
    <div className="bg-slate-50/50 border border-slate-200/50 rounded-2xl p-2 md:p-3">
      ...
      <div className="block md:hidden space-y-3">
        {transactions.map((tx) => {
          ...
          return (
            <div key={tx.id} className="bg-white border border-slate-100/80 rounded-2xl p-4 shadow-sm flex flex-col gap-3">
              {/* Línea Superior: Descripción e Importe */}
              <div className="flex justify-between items-start gap-2">
                <div className="flex flex-col gap-1 min-w-0">
                  <span className="font-bold text-slate-800 text-[13px] truncate">{tx.description}</span>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{formatDate(tx.date)}</span>
                </div>
              ...
              <div className="flex justify-between items-center pt-2.5 border-t border-slate-50 gap-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {/* Cuenta */}
                  {tx.bank_account_name ? (
                    <div className="flex items-center gap-1 bg-slate-100 text-slate-655 border border-slate-200/50 px-1.5 py-0.5 rounded text-[10px] font-bold">
